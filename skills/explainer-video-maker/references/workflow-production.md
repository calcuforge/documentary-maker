...
**Prereq:** `project_prefs.tts.voice_file` must point to an existing reference audio file — normally `projects/{p}/voice_reference.wav` (generated by Step 0 voice design). IndexTTS uses it to clone the speaker's timbre.

### (b) HTTP TTS server (multipart/form-data)

```bash
python3 "$SKILL_DIR/scripts/cli.py" tts run --project $P --video $V --backend http_server
```

POSTs `multipart/form-data` with fields `input=<narration text>`, `voice_file=@...`, `speed=1.0`. Requires `BACKEND_PROXY_ENDPOINT` env var. Saves response body → `narration_audio.wav`.
